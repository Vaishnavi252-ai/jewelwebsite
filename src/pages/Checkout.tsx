    ...
    try {
      const order = await placeOrder({
        ...
        cartItems: items.map(i => ({
          ...
          quantity: i.quantity,
          unit_price: i.price,
          product: { title: i.title, images: i.images },
        })),
                  ...
                  return (
                    <div key={item.id} className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p style={{ color: '#2B2B2B' }} className="text-sm font-medium truncate">{item.title}</p>
                        <p style={{ color: '#6B6B6B' }} className="text-xs">Qty: {item.quantity}</p>
                        <p style={{ color: '#B9375E' }} className="text-sm font-semibold">₹{(item.price * item.quantity).toLocaleString('en-IN')}</p>
                      </div>
                    </div>
                  );
                })}
